...
export const TopicCardSchema = z.object({
  ...
  news_brief: z.string().min(1).max(120),
  impact_axis: z.enum(["注册", "入金", "交易"]),
  severity: Severity,
  impact_register: ImpactLabel,
  impact_register_score: z.number().int().min(0).max(100),
  impact_deposit: ImpactLabel,
  impact_deposit_score: z.number().int().min(0).max(100),
  impact_trading: ImpactLabel,
  impact_trading_score: z.number().int().min(0).max(100),
